/**
 * GHL Login Test - Headless POC
 * Tests actual Playwright login to GHL dashboard
 * 
 * Usage: node login-test-headless.js
...
async function loginToGHL() {
  console.log('🚀 Starting GHL Login Test (Headless Mode)\n');
  ...
  try {
    ...
    console.log('[1/6] Launching browser (headless)...');
    ...
    const loginButton = page.locator('button:has-text("Login"), button:has-text("Sign In"), button:has-text("Continue")').first();
    ...
    try {
      ...
      console.log('(Dashboard URL pattern not matched, checking for elements...)');
    }

    // Wait for any page content to load
